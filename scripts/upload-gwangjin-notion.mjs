#!/usr/bin/env node
/**
 * 광진구 공단주소 데이터를 노션 에디토리얼 DB에 업로드하는 스크립트
 */
...
const data = JSON.parse(fs.readFileSync('data/공단주소/광진구.json', 'utf-8'));
const gu = '광진구';
...
async function main() {
    ...
    for (const lot of lots) {
      ...
      if (feeData) {
        ...
        if (categoryKey === '노상주차장') {
          ...
          if (feeData['시간제'] && feeData['시간제'] !== '-') parts.push(`시간제: ${feeData['시간제']}`);
          if (feeData['비고']) parts.push(feeData['비고']);
        }
        feeText = parts.join(' / ');
      }
  ...
  console.log(`기존 광진구 노션 페이지: ${existingPages.size}개\n`);
