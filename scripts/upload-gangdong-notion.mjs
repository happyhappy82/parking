#!/usr/bin/env node
/**
 * 강동구 공단주소 데이터를 노션 에디토리얼 DB에 업로드하는 스크립트
 */
...
const data = JSON.parse(fs.readFileSync('data/공단주소/강동구.json', 'utf-8'));
const gu = '강동구';
...
async function main() {
  ...
  console.log(`기존 강동구 노션 페이지: ${existingPages.size}개\n`);
